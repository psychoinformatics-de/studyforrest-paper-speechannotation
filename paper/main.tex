...
For this publication, we annotated the onsets and offsets, and content of sentences, single words, and phonemes (cf. table \ref{tab:overview} for an overview of the annotation's content) spoken in the audio-visual movie ``Forrest Gump'' \citep{ForrestGumpMovie} and its audio-description (i.e. the audio-only variant of the movie) for visually impaired persons \citep{ForrestGumpDVD}.
...
The dictionary was manually updated with German words that occur in the stimuli but were missing in the dictionary to improve the detection rate of the automatic alignment.
...
In a first step, onsets and offsets words and phonemes on which the automatic alignment performed moderately were corrected in Praat.
...
In a second step, the complete sentences of the orthographic transcription were copied into the annotation.
...
\begin{table*}[tbp]
\caption{Overview of the annotation's content for the whole stimulus and its individual segments used during fMRI scanning. The label ``Sentences'' comprises complete grammatical sentences (i.e. comprising subject predicate) which are additionally marked with a full stop at the end.
The label ``Sentences'' also comprises questions (``Do you want a chocolate?'', exlamations (``Run away!), or non-speech vocalizations in quick succession (``ha, ha, ha''), or in isolation (e.g. ``Forrest?'', ``Forrest!'', ``ha'') at time points when speakers switch rapidly. The label ``Words'' comprises each word or non-speech vocalization (N=\aPosNonspeechAll) in isolation.}
\label{tab:overview}
...
\begin{table*}[btp]
\caption{Sentences spoken by the ten most often occurring speakers sorted alphabetically. The narrator only occurrs in the audio-description of the movie. Names are mostly identical to the names used in \citep{labs2015portrayed}. Overall 97 persons were identified.}
\label{tab:speakers}
\begin{tabular}{llllllllll}
\toprule
...
(\href{https://spacy.io}{spacy.io}) employing its accompanying German language model (\texttt{de\_core\_news\_md} trained on the TIGER Treebank corpus \citep{brants2004tiger}.
Non-speech vocalizations were dropped from the sentences before analysis.
We performed analyses regarding part-of-speech (i.e. grammatical tagging or word-category disambiguation), syntactic dependencies, lemmatization, word embedding (i.e. semantic similarity), and if the word is one of the most common words of the German language, i.e. if the word is part of a stop list.
...
% einige Eigennamen (NE/NN) wurden automatisch durch das Skript korrigiert, weil das Sprachmodell die naheliegenderweise nicht kannte
% eine Liste dieser Eigennamen befindet sich oben in dem entsprechendem Skript
...
See Table \ref{tab:tag} for a descripion  of the labels and the respective counts of the 15 most often occuring labels.
...
Information about a word's syntactic dependencies taking the relationship with other words in the same sentence into account.
...
We used fMRI BOLD data of the 2h audio-description (7 Tesla, 2s TR, 3599 volumes, partial brain coverage with 1.4 mm isotropic voxels) \citep{hanke2014audiomovie} that were already corrected for motion and aligned [+++wie?+++] to study specific group template [+++co-registered to MNI?+++].
...
% 1st lvl
At the first level analyzing each segment for each subject individually, we created 26 regressors (s. table \ref{tab:regressors}) based on events drawn from the annotation.
The 20 most often occuring detailed tag labels (``nn'' with N=\rNnAll\space to ``prf'' with N=\rPrfAll) were modelled as such from the onset to offset of each word. The remaining tags were pooled to (``tag\_other``; N=\rTagotherAll).
...
The ends of a complete grammatical sentences were used as onsets of a event abitrarily lasting 0.2s, for the regressors ``sentence'' (N=\rSentenceAll).
``No-speech'' events (``no-sp''; N=\rNospAll) serving as a control condition were created by the following rationale:
events were fitted into intervals between onsets and offsets of sentences/words assumed to contain no audible/comprehensible speech.
Each event of the no-speech condition had a minimum distance of 1.8s to onsets/offsets of a sentence or an isolated word, and to each onset to another event of the no-speech condition.
A length of 0.07s was chosen for no-speech events matching the average length of phonemes.
Lastly, we used events reflecting low-level auditory features (left-right difference in loudness and root mean squared loudness) aligned to and averaged across the length of every movie frame (40 ms) to model nuisance effects.
...
The Pearson correlation coefficients of the 26 regressors across the time course of the stimulus can be seen in Figure \ref{fig:reg-corr}.
...
% hier die Lateralisation in den Broca Arealen BA44 und BA45 klarer machen
\begin{table*}[t]
...
\todo[inline]{Decide on having a 'conclusion' or not. If the section is not
  mandatory and there are no further conclusions than at the end of the
  introduction then I am OK with not having one. However, the paper needs to
  serve a purpose. One is, of course, to be documentation for the annotation.
  Another is to attract people to use this work for their own studies. I would
  even argue that this is the \textit{main} purpose, given that the annotation
  in itself represents to scientific progress. So the paper need to give an
  ...
  modelling of hemodynamic brain responses correlating with speech-related
  events' and the only indication of purpose is 'additional confound measures'. I
  think this is incongruent with the amount of work and the level of detail of
  this annotation.}
