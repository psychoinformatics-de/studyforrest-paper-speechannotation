...
\todo[inline]{this work seems to improve/replace previous work (in part), it is important to state that clearly and identify which pieces are involved}
...
Preliminary, orthographic transcriptions of dialogues (\texttt{german\_dialog.csv})\todo{filenames should have some text markup. I did this example, please apply to other locations as well},
non-speech vocalizations (e.g. laughing or groaning;
non\_speech\_vocalization.json), and the audio-description's narrator
(german\_audio\_description.csv)\todo{where were those from? citation missing} were merged and converted to Praat's
...
improvements\todo{in general process information is not relevant: of course you need to open a file to work with it. The Q is what you did to it, why, and how exactly. So instead of "was opened along (the audio file of) the audio-description in Praat v6.0.22", better "was edited in Praat v6.0.22 for manual validation against the original audio"}: In several passes\todo{why several passes?}, the approximate temporal onsets and offsets
...
a regular audience\todo{what is a regular audience? without neurological disorders ;-)} were dropped, too.

% Alignment preparations
We used the Montreal Forced Aligner v1.0.1 
\href{montrealcorpustools.github.io/Montreal-Forced-Aligner}{montrealcorpustools.github.io/Montreal-Forced-Aligner}\citep{mcauliffe2017montreal}\todo{we need a consistent way of citing software. I think it is overkill to have a reference, plus a URL, plus a version in the main text. Either everything in the reference, or (maybe) just name+version in the text};to
...
the Prosodylab PhoneSet\todo{same here, use a proper bibtex reference}. The dictionary was manually updated with movie-specific
German words\todo{which words, how determined, why?}. The pronunciation of English words occurring in the otherwise
German audio track was taken from an English pronunciation dictionary
(\href{http://mlmlab.org/mfa/dictionaries/english.dict}{mlmlab.org/mfa/dictionaries/english.dict} that uses the ARPAbet PhoneSet\todo{It would help a non-expert like me to understand why a PhoneSet is important for this annotation, sounds like a jargon}.
...
The audio file was converted from .FLAC to .WAV\todo{Better WAV not .WAV, likewise for others} via ffmpeg v4.1.4
