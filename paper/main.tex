...
part-of-speech-tagging putting words into 15 grammatical categories, a detailed
part-of-speech tagging putting words into 43 grammatical categories, syntactic
dependencies within each sentence, and a semantic analysis based on word
embeddings representing each word in a 300-dimensional semantic space.
...
corresponding fMRI data. However, data-driven methods alone fall short of
pinpointing their results to specific, time-locked features of a stimulus
\citep{kauttonen2015optimizing}.
% model-driven methods
Model-driven methods, like a general linear model (GLM), can be useful to
investigate how hemodynamic brain responses in isolated regions correlate with
stimulus features. They can explain not just ``how'' the brain is responding to
a stimulus but also ``why'' \citep{hamilton2018revolution}.
% GLM: example studies
...
lab and now provide one single annotation with exact onsets/offsets and content
of each sentences, word and phonemes (see Table \ref{tab:overview} for an
overview) spoken in the audio-visual movie ``Forrest Gump''
\citep{ForrestGumpMovie} and its audio-description (i.e. an audio-only variant
of the movie)\citep{ForrestGumpDVD}.
...
% why
This publication enables researchers to model time-locked hemodynamic brain
responses that correlate with different aspects of spoken occurring in a 2-hour
lasting naturalistic stimulus.
...
measures describing key properties of major building blocks of the stimulus.

\todo[inline]{Das “why one should model” ist jetzt drin; Frage an dich als
Datenanalyse-Pro: Man kann doch bestimmt auch data-driven methods irgendwie
“constrainen”, und die durch eine Annotation verbessern? Hast du da eine
Formulierung und/oder auch ein (high-level) Paper für mich zu dem Thema?}
...
algorithmically identify the exact onset of offset of the sentences, and the
onset and offset of each word and phoneme embedded in the transcribed sentences.
...
% spaCy
To automatically analyze linguistic features of each words in their
corresponding sentence, we employed the Python package spaCy \citep{spacy2} and
its accompanying German language model (\texttt{de\_core\_news\_md} that was
trained on the TIGER Treebank corpus \citep{brants2004tiger}.
...
improve results.
% performed analyses
We performed analyses regarding part-of-speech (i.e. grammatical tagging or
word-category disambiguation), syntactic dependencies, lemmatization, word
embedding (i.e. multi-dimensional meaning representation of a word), and if the
word is one of the most common words of the German language (i.e. if the word is
part of a stop list).


\subsection*{Data Legend} \todo[inline]{Minimale Info hier, dass es zwei
Versionen gibt und sich der folgende Abschnitt auf TSV file bezieht}
% TextGrid vs. TSV
...
part-of-speech tagging (grammatical tagging; word-category disambiguation) of a
word.
% annotation scheme
The simple part-of-speech tagging tags follow the Universal Dependencies v2 POS
tag set (\href{https://universaldependencies.org}{universaldependencies.org}).
% reference to table
See Table \ref{tab:pos} for a description of the labels and the respective
counts of all 15 labels. Nouns that spaCy mistook for proper nouns or vice
versa were corrected via script.
...
A detailed part-of-speech tagging of a word following the TIGER Treebank
...
counts of the 15 most often occuring labels. Nouns that spaCy mistook for proper
nouns or vice versa were corrected via script.
% s. list at the beginning of script code/add_part-of-speech-tagging2textgrid.py
...
from onset to offset of each word \todo{This has errors that I do not know how
to fix; sorry, unclear what you mean by that}.
...
function from onset to offset of the corresponding word\todo{I do not know what
this mean; CH: hope it got clearer now}.
% phonemes
The top 80 phonemes (\texttt{n} with N=6053 to \texttt{IY1} with N=32) were
pooled to \texttt{phonemes} (N=\rPhonesAll) and each event modeled as boxcar
function from onset to offset of the corresponding phoneme.\todo{Does that mean
they share events with "other"; CH: dunno what you mean, but text should be
clearer now}.
% sentence (endings)
The end of each complete grammatical sentence was modeled as an impulse event
(N=\rSentenceAll) to capture variance of sentence comprehension\todo{Why? Above
it says 'we tested if similar linguistic concepts providing high semantic
information (proper nouns and nouns)'. What does this need sentences for? CH:
it is kinda nuisance regressor}.
% no-speech intro
...
intervals without audible speech \todo{Why assumed? Isn't the point of the
annotation to "know"? CH: words where chosen to carefully; intervals don't
contain any audible speech with >98 perc. certainty}
...
Lastly, we used events\todo{They are not really events, but a continuous
nuisance regressor; CH: okay, how do you phrase that correctly?} reflecting
low-level auditory features (left-right difference in volume and root mean
squared volume) averaged across the length of every movie frame (\unit[40]{ms}).
All events were convolved with FSL's ``Double-Gamma HRF'' as a model of the
hemodynamic response function to create the actual regressors.
...
Temporal derivatives were also included in the design matrix and the design was
subjected to the same temporal filtering as the BOLD time series.
% motion parameters
...
carried out using FLAME stage 1 with automatic outlier de-weighting
...
% COPE 1:
The contrast words (all 21 \texttt{tag-related} regressors) > no-speech yielded
four significant clusters (see Table \ref{tab:cope1}):
% cluster 1
...
% COPE 3
The contrast proper nouns > coordinate conjunctions yielded 9 significant
...
% COPE 5
The contrast nouns > coordinate conjunctions yielded 4 significant clusters (see Table \ref{tab:cope5}):
% cluster 1 & 2
two clusters that are slightly smaller than lateral temporal clusters of contrast nouns > coordinate conjunction. In this case, spanning from angular gyrus in the left hemisphere and from planum temporale in the right hemisphere into the anterior part of superior temporal cortex.
% cluster 3 & 4
Finally, two small right lateralized clusters in the right posterior cingulate
gyrus and right precuneus.

\todo{“partial brain coverage” / FoV. MIH: Could become an issue. In the data
paper we showed the coverage. Maybe just ref the figure. CH: gonna take a look how I can adjust the script to get a FoV mask from the data and use it during plotting; otherwise: ref to paper}
\begin{figure*}
  \centering
...
\todo[inline]{Lass uns bzgl. der Discussion am besten unterhalten; im Grunde
geht es darum, wie weit ich ins Detail gehen soll (kann nämlich viel zu sehr ins
Detail gehen); im Grunde geht es imo nicht darum, alle Cluster inkl. Areale
durchzudiskutieren, sondern zu kommunizieren, dass die Anno brauchbar ist; wie
dem auch sei: Hier wird noch Inhalt nach dem Lesen der Reviews hinzukommen}
% words > no-speech
In the contrast words > no-speech, we find increased hemodynamic activity in a
...
Our results are similar to results of studies that implemented an ISC approach
to analyze fMRI data of naturalistic auditory stimuli \citep{honey2012not,
lerner2011topographic, silbert2014coupled}.
% dissimilarities
Contrary to previous studies, we do not find increased medial activation in the
precuneus or medial frontal cortex. This could be attributed to the fact that
our contrast does not \todo{Which contrast captures ANY social aspects? CH: the
COPEs in the references} capture social aspects of the narrative
\citep{ferstl2008extended, mar2011neural}\todo{check it}.
% (proper) nouns > con
The two contrasts contrasting nouns and proper nouns respectively to coordinate
junctions lead to unexpected results\todo{That seems like an unfortunate
statement. We make this analysis to show expected results to conclude that the
annotation is good. But we get unexpected results and conclude; CH: the contrast
turned out to yield the result; cf. following explanation; I don't feel
comfortable of HARKing}.
% cluster location
Increased activation is partially located in early sensory regions (Heschl's
Gyrus; \citep{saenz2014tonotopic}) and most prominently adjacent regions
bilaterally (planum temporale; superior temporal gyrus;
...
This could be attributed to the fact that a high count of heterogeneous nouns
(N=\rNnAll) and proper nouns (N=\rNeAll) were averaged across all 300 semantic
dimensions (cf. \citep{mitchell2008predicting, huth2016natural}), and contrasted
to coordinate conjunctions that offered a comparably small amount of events
(N=\rKonAll).
% interpretation 1b: not just narrow concept but also dominated by just two
% different words
Moreover, the coordinate junctions (\texttt{kon}) mostly comprises event of the
words ``and'' (N=392) and ``but'' (N=49) and thus offered only a small amount of
variance.
% interpretation 2
A second explanation might be that our phoneme regressors that assumed linear
summation of approximately 70 ms lasting events in quick succession did not
capture a sufficiently amount of nuisance variance\todo{Reviewer 2 will ask: If
you knew all these issues, why did you do it then? hindsight bias/HARKing after
exploratory analyses?!}.
% but...
Nevertheless and more importantly to access the validity of the dataset is the
fact that two similar linguistic concepts (nouns and proper nouns) embedded in a
naturalistic stimulus that was not designed for linguistic research lead to
similar activation patterns\todo{That statement is only true, if both contrasts
are not (solely) driven by the conjunctions. There is no evidence in the paper
that this is not the case.}
% Within the language processing systems, early sensory regions are specialized
% for representing the immediate physical properties of the environment (form),
% while higher order regions extract increasingly abstract content (Davis and
% Johnsrude, 2003; Quiroga et al., 2005; Okada et al., 2010).
...
\section*{Conclusions} \todo[inline]{the paper needs to serve a purpose.  One
is, of course, to be documentation for the annotation.  Another is to attract
people to use this work for their own studies.  I would even argue that this is
the \textit{main} purpose, given that the annotation in itself represents to
scientific progress.  The need to give an accurate assessment what this can be
used for and what is problematic or impossible.  ATM this is the only statement
in this regard: 'facilitates modeling of hemodynamic brain responses correlating
with speech-related events' and the only indication of purpose is 'additional
confound measures'.  I think this is incongruent with the amount of work and the
level of detail of this annotation; CH: let's discuss that personally}
% state what you think are the main conclusions that can be realistically drawn
% from the findings in the paper, taking care not to make claims that cannot be
% supported.  intro
...
% what for
An additional tagging of each word's linguistic features and the public nature
of the dataset enable independent working groups to create sophisticated models
of hemodynamic activity correlating with speech-related events in a naturalistic
stimulus.
% results
