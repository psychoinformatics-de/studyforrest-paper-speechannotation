...
% 1st lv}
\todo[inline]{What follows has typos}
...
The 20 most often occuring detailed part-of-speech labels (\texttt{nn} with N=\rNnAll\space to text{prf} with N=\rPrfAll) were modelled as such from the onset to offset of each word\todo{This has errors that I do not know how to fix}.
The remaining labels were pooled to (\texttt{tag\_other}; N=\rTagotherAll)\todo{I do not know what this means}.
The top 80 phonemes (\texttt{n} with N=6053 to \texttt{IY1} with N=32) were pooled to \texttt{phonemes} (N=\rPhonesAll) to be used as a nuisance regressors\todo{Does that mean they share events with "other"}.
The end of each complete grammatical sentence was modeled as an impulse event (N=\rSentenceAll)\todo{Why? Above it says 'we tested if similar linguistic concepts providing high semantic information (proper nouns and nouns)'. What does this need sentences for?}.
``No-speech'' events (\texttt{no-sp}; N=\rNospAll) serving as a control condition were created by the following rationale:
events were fitted into longer lasting intervals between onsets and offsets of words assumed\todo{Why assumed? Isn't the point of the annotion to "know"?} to contain no audible speech.
...
Lastly, we used events\todo{They are not really events, but a continous nuisance regressor} reflecting low-level auditory features (left-right difference in volume and root mean squared volume) averaged across the length of every movie frame (\unit[40]{ms}).
