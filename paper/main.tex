...
\todo[inline]{Bitte am Ende (?) der Einleitung etwas zu "reproducible research" \& "datalad dataset" bringen}
...
\todo[inline]{In der Tabelle ausführlicher beschrieben als hier (ist okay so, denke ich): ``sentences'' sind hier nicht vollständige Sätze im linguistischen Sinne (Subjekt, Prädikat), sondern auch Ausrufe ("Forrest!") oder mitunter isolierte non-speech "Sätze" ("oouh")}
For this publication, we annotated the onsets and offsets, and content of sentences, single words, and phonemes spoken in the audio-visual movie ``Forrest Gump'' \citep{ForrestGumpMovie} and its audio-description (i.e. the audio-only variant of the movie)\citep{ForrestGumpDVD}(cf. table \ref{tab:overview} for an overview of the annotation's content).
...
\todo[inline]{was meinst du mit: this work seems to improve/replace previous work (in part), it is important to state that clearly and identify which pieces are involved}
...
\todo[inline]{wo kommen die "preliminaries transcriptions" etc.? Habe die von Medusa; die "most basic" Datei, die das Dataset enthält ist wie abgesprochen der Merge aus Dialog (ungetrennte Sentences), Narrator \& Non-Speech-Annotation = \texttt{annotation/speech-vocal.csv}}
...
(\texttt{german\_audio\_description.csv}) were merged (\texttt{annotation/speech-vocal.csv}) and converted to Praat's \citep{boersma2019praat} TextGrid format  (\texttt{annotation/speech-vocal.TextGrid}).

% \href{www.fon.hum.uva.nl/praat}{fon.hum.uva.nl/praat}.
The merged transcription containing a rough onset and offset for usually a couple of sentences was edited in Praat v6.0.22 for manual validation against the original audio.
...
\todo[inline]{what is a regular audience? without neurological disorders ;-); Simpel: eine audience, die den Film nicht 45x auf voller Lautstärke über Kopfhörer mit Transkription danebenliegend gehört hat}
...
% Alignment
\todo[inline]{Nicht erwähnt: Wenn die Korrektur der Phoneme unmöglich war, habe ich einfach die Event-Borders zwischen den Phonemen herausgenommen. Wortbeginn und Wortende stimmen dann mit einem neuen, "künstlich-großen Phoneme" überein.
...
The audio track was converted from FLAC to WAV via FFmpeg v4.1.4 \citep{ffmpeg} to meet the aligner's requirements.
...
In a third step, the a speaker's identity was added for each sentence (cf. Table \ref{tab:speakers} for the most often occurring speakers).
During every step results from previous steps were checked again for errors.

\begin{table*}[tbp]
\caption{Overview of the annotation's content for the whole stimulus and its individual segments used during fMRI scanning. The label ``sentences'' comprises complete grammatical sentences which are additionally marked in the annotation with a full stop at the end (``my feet hurt.'').
It also comprises questions (``do you want a chocolate?''), exlamations (``run away!), or non-speech vocalizations in quick succession (``ha, ha, ha''), or in isolation (e.g. ``Forrest?'', ``Forrest!'', ``ha'') at time points when speakers switch rapidly. The label ``words'' comprises each word or non-speech vocalization (N=\aPosNonspeechAll) in isolation.}
\label{tab:overview}
...
(\href{https://spacy.io}{spacy.io}) and its accompanying German language model (\texttt{de\_core\_news\_md} that was trained on the TIGER Treebank corpus \citep{brants2004tiger}.
...
Results of these analyzes were added to the annotation which is available as a (text-based) Praat TextGrid file (\texttt{annotation/fg\_rscut\_ad\_ger\_speech\_tagged.TextGrid} and as a text-based, tab-separated value (TSV) formatted table (\texttt{annotation/fg\_rscut\_ad\_ger\_speech\_tagged.tsv}) in accordance with the brain imaging data structure (\href{https://bids.neuroimaging.io/}{BIDS}) \citep{gorgolewski2016bids}).
\todo[inline]{hier bringe ich bereits vor der ``Data availability'' die Pfade/Dateinamen, weil im Folgenden die TSV-Datei in der ``Data legend'' beschrieben wird}


\subsection*{Data Legend}
The annotation in TextGrid format comprises 10 tiers: person, sentence (the text of a sentence), words (the text of a word), pos, tag, dep, lemma, stop, vector and phones (pronounciation of a phoneme).
The annotation in TSV format comprises 11 columns: onset, duration, person, text (of sentence, word or phoneme), pos, tag, dep, lemma, stop, vector.
...
Additionally in cells of this column, sentences are tagged as "SENTENCE", and phonemes are tagged as "PHONEME" to facilitate sorting, and filtering in potential further processing steps.

\begin{table*}[t]
\caption{Simple part-of-speech tagging (pos) performed by the Python package spaCy (\href{https://spacy.io}{spacy.io}).
...
\toprule
textbf{label} & \textbf{description} & \textbf{all} & \textbf{1} & \textbf{2} & \textbf{3} & \textbf{4} & \textbf{5} & \textbf{6} & \textbf{7} & \textbf{8} \\
\midrule
...
\begin{table*}[t]
\caption{Detailed part-of-speech tagging (tag) performed by the Python package spaCy (\href{https://spacy.io}{spacy.io}).
...
\begin{table*}[t]
\caption{Syntactic dependencies (dep) performed by the Python package spaCy (\href{https://spacy.io}{spacy.io}).
The 15 most often occuring labels (overall 37 labels) sorted alphabetically.
Descriptions were taken from spaCy.explain()(\href{https://spacy.io}{spacy.io}).
Counts for the whole stimulus (all) and for each of the eight stimulus segments refer to the audio-description.}
\label{tab:dep}
\begin{tabular}{lllllllllll}
\toprule
...
\todo[inline]{darauf gehe ich schon am Ende von “annotation procedure” vor Datalegend ein, weil ich bereits dort erwähne, dass es zwei Versionen gibt und sich die Datalegend auf die TSV file bezieht.}
...
% Preprocessing
\todo[inline]{die Daten aus Hanke2014 sind nicht bewegungskorrigiert und nicht  aligned. Quelle ist in vorliegendem Falle:
juseless.inm7.de:/data/project/studyforrest/collection/phase1}
\todo[inline]{bei allem, was folgend mit alignment/co-registration zu tun hat, bin ich mir nicht sicher \& gehe ggf. zu wenig auf den Prozess der Template Erstellung und des alignments ein; vgl. Review zum Muscial Genre Paper}
\todo[inline]{hanke2014 spricht von 20 subjects (age 21–38 years, mean age 26.6 years, 12 male), es fehlt hier aber sub-10}
We used fMRI BOLD data of the 2h audio-description (\unit[7]{Tesla}, TR=\unit[2]{s}, 3599 volumes, 36 axial slices, thickness \unit[1.4]{mm}, \unit[1.4 $\times$ 1.4]{mm} in-plane resolution, \unit[224]{mm} field-of-view) of 19 healthy subjects \citep{hanke2014audiomovie} that were already corrected for motion and aligned [+++wie?+++] to study specific group template [+++co-registered to MNI?+++].
...
\todo[inline]{what does e1-12 stand for? It makes the label column too wide; das waren hline Befehle}
...
The label ``sentence'' contains the end of complete grammatical sentences. label ``phones'' contains events of the 80 most often occuring phonemes (``n'' with N=6053 to ``IY1'' with N=32).
The label ``no-sp'' represents moments when no speech was audible. ''fg\_ad\_lrdiff (left-right volume difference) and ``fg\_ad\_rms'' (root mean square loudness) represent one event for every movie frame (40ms). Events were convolved with FSL's ``Double-Gamma HRF'' to create the regressors. The correlation of these regressors over the time course of the whole stimulus can be seen in Figure \ref{fig:reg-corr}.}
...
At the first level analyzing each segment for each subject individually, we created 26 regressors based on events drawn from the annotation (cf. Table \ref{tab:regressors}).
The 20 most often occuring detailed part-of-speech labels (``nn'' with N=\rNnAll\space to ``prf'' with N=\rPrfAll) were modelled as such from the onset to offset of each word.
The remaining labels were pooled to (``tag\_other``; N=\rTagotherAll).
The top 80 phonemes (``n'' with N=6053 to ``IY1'' with N=32) were pooled to  ``phonemes'' (N=\rPhonesAll) to be used as a nuisance regressors.
...
``No-speech'' events (``no-sp''; N=\rNospAll) serving as a control condition were created by the following rationale:
events were fitted into longer lasting intervals between onsets and offsets of words assumed to contain no audible/comprehensible speech.
Each event of the no-speech condition had to have a minimum distance of 1.8s to an onsets/offsets of word, and to each onset to another event of the no-speech condition.
A length of \unit[70]{ms} was chosen for no-speech events matching the average length of phonemes.
Lastly, we used events reflecting low-level auditory features (left-right difference in loudness and root mean squared loudness) averaged across the length of every movie frame (\unit[40]{ms}).
...
\todo[inline]{in den Results/Discussion wird nur auf die COPEs 1, 3, 5 eingegangen; sollte man hier 2, 4, 6 rausnehmen? oder von “and their reverse contrasts” sprechen?}
The following six t-contrasts were defined:

% events:n all tag-related regressors: N=15985)
1) words (all 21 tag-related regressors) > no-speech;
2) no-speech > words;
3) proper nouns (``ne''; N=\rNeAll) > coordinate conjunctions (``kon''; N=\rKonAll);
4) coordinate conjunctions > proper nouns;
5) nouns (``nn''; N=\rNeAll) > coordinate conjunctions, and
...
% Results
% COPE 1:
Figure \ref{fig:results} depicts the results of the three \todo{drei = “primary three”} contrasts (all Z-Threshold Z>3.4; p<.05 cluster-corrected).
The contrast words > no-speech yielded four significant clusters (cf. Table \ref{tab:cope1}):
...
% COPE 3
The contrast proper nouns > coordinate conjunctions yielded 9 significant clusters (cf. Table \ref{tab:cope3}):
...
% COPE 5
The contrast nouns > coordinate conjunctions yielded 3 significant clusters (cf. Table \ref{tab:cope5}):
...
\begin{table*}[t]
\caption{Significant clusters (Z-Threshold Z>3.4; p<.05 cluster-corrected) for the contrast words > no-speech.
...
\begin{table*}[t]
\caption{Significant clusters (Z-Threshold Z>3.4; p<.05 cluster-corrected) for the contrast proper nouns (``ne'') > coordinate conjunctions (``kon'').
Clusters sorted by voxel size.
The first brain structure given contains the voxel with the maximum Z-Value, followed by brain structures from posterior to anterior, and partially covered areas.}
\label{tab:cope3}
\begin{tabular}{rrrrrrrrrp{6cm}}
...
\begin{table*}[t]
\caption{Significant clusters (Z-Threshold Z>3.4; p<.05 cluster-corrected) for the contrast nouns (``nn'') > coordinate conjunctions (``kon'').
Clusters sorted by voxel size.
The first brain structure given contains the voxel with the maximum Z-Value, followed by brain structures from posterior to anterior, and partially covered areas.}
\label{tab:cope5}
\begin{tabular}{rrrrrrrrrp{6cm}}
...
In the contrast words > no speech, we expectedly find increased hemodynamic activity in a bilateral cortical network including temporal, parietal and frontal regions realted to processing spoken language \citep{hickok2007cortical}.
...
Moreover, the label ``coordinate junctions'' mostly comprises event of the words ``and'' (N=392) and ``but'' (N=49) and thus offered only a small amount of variance.
...
I would even argue that this is the \textit{main} purpose, given that the annotation in itself represents to scientific progress. So the paper need to give an  accurate assessment what this can be used for and what is problematic or
impossible.
