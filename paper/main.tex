...
of pinpointing their results to specific, time-locked stimulus features \citep{kauttonen2015optimizing}.
...
identity, to phonology, grammar, and semantics.
...
% \href{https://www.ims.uni-stuttgart.de}{ims.uni-stuttgart.de}.  dropping of
% non-speech vocalizations because they often delude the algo
...
to fix; COH: sorry, unclear what you mean by that}.
...
modeled as boxcar function from onset to offset of the corresponding
phoneme.\todo{Does that mean they share events with "other"; CH: dunno what you
mean, but text should be clearer now}.
% sentence (endings)
...
A length of \unit[70]{ms} was chosen for no-speech events matching the
average length of phonemes.
% low-level nuisance regressors
Lastly, we used continuous information about auditory features (left-right
difference in volume and root mean squared volume) that was averaged across the
length of every movie frame (\unit[40]{ms}) to capture variance correlating with
...
% motion parameters & temporal filtering of design
% motion parameter are filtered, too. See FSL manual:
% ``All (confound) EVs will be filtered to match the processing applied to the
% input data''
Finally, six motion parameters were used as additional nuisance regressors and
the design was subjected to the same temporal filtering as the BOLD time
series.
...
For the contrast words > no-speech, we find increased hemodynamic activity in a
...
% dissimilarities
\todo{Which contrast captures ANY social aspects? CH: the COPEs in the
references} Contrary to previous studies \citep{ferstl2008extended,
mar2011neural}, we do not find increased medial activation in the precuneus or
medial frontal cortex. This could be attributed to the fact that the contrast
here was not designed to capture social aspects of the narrative.\todo{check the
previous studies}.
% (proper) nouns > con
The two contrasts contrasting nouns and proper nouns respectively to coordinate
junctions yielded increased activation that is partially located in early
...
not capture a sufficiently amount of nuisance variance\todo{Reviewer 2 will ask:
    If you knew all these issues, why did you do it then? hindsight bias/HARKing
after exploratory analyses?!}.
% but...
Nevertheless and more importantly to exploratorily access the validity of the
dataset is the fact that two similar linguistic concepts (nouns and proper
nouns) embedded in a naturalistic stimulus that was not designed for linguistic
research lead to similar activation patterns\todo{That statement is only true,
if both contrasts are not (solely) driven by the conjunctions. There is no
evidence in the paper that this is not the case.}
